...
Vessel Enhancement Filtering. In Wells, WM, Colchester, A, & Delp, S,q
...
class ComputeVED:
    ...
    def run(self):
        ...
        if (
            self._generate_iteration_files
            or self._generate_scale
        ...
        if self._generate_iteration_files or self._frangi_only:
            vesselness_path = os.path.join(
                self._out_folder, (prefix + "frangi_only_vesselness" "_measure.nii.gz")
            )
            files_created_ved.append(vesselness_path)
            shorts.append("frangi_only_vesselness_measure.nii.gz")

        if self._generate_scale:
            best_scale_path = os.path.join(
                self._out_folder, (prefix + "ved_generated_" "best_scale.nii.gz")
            )
            files_created_ved.append(best_scale_path)
            shorts.append("ved_generated_best_scale.nii.gz")

        if self._generate_hessian:
            best_hessian_path = os.path.join(
                self._out_folder, (prefix + "ved_generated_" "best_Hessian.nii.gz")
            )
        ...
        if self._generate_iteration_files:
            iteration_path = [
                os.path.join(
                    self._out_folder, (prefix + "ved_iteration" "_{}.nii.gz".format(i))
                )
            ...
            shorts_path = [
                os.path.join("ved_iteration_{}.nii.gz".format(i))
                for i in range(1, self._number_iterations + 1)
            ]
...
if __name__ == "__main__":
    ...
    parser.add_argument("input", type=str, help="The input file name (relative path)")
    parser.add_argument("output", type=str, help="The output file name (relative path)")
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="The number of diffusion iterations. " "[default: 1]",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=5.0,
        help="The sensitivity used in VED param. " "[default: 5.0]",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=25.0,
        help="The weight strength used in VED " "equation. [default: 25.0]",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.1,
        help="The epsilon used in VED equation. " "[default: 0.1]",
    )
    ...
    parser.add_argument(
        ...
        "--scale_object",
        action="store_true",
        help="Flag to rescale vesselness measure " "based on eigen amplitude.",
    )
    ...
    parser.add_argument(
        ...
        "--generate_scale",
        action="store_true",
        help="Flag to generate an output for the " "best sigma value per voxel.",
    )
    ...
    parser.add_argument(
        ...
        "--generate_hessian",
        action="store_true",
        help="Flag to generate an output with the " "hessian matrix per voxel.",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Flag to generate output iteration " "files and vesselness.",
    )
