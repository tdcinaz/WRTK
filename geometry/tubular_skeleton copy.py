...
class Skeleton(pv.PolyData):
    ...
    def find_potential_at_point(self, artery, coords: np.ndarray, print_field=False):
    ...
    *Knots* (explicit artery control points) can be moved interactively; all
    ...
    def cost(self, artery, alpha=1):
        ...
        for idx, knot_point in enumerate(knot_points):
            r_vectors = knot_point - knot_points
            dists = np.linalg.norm(r_vectors, axis=1)
            valid_mask = dists > 0
            dists = dists[valid_mask]
            potentials[idx] += np.sum((alpha) / dists)
